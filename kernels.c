...
int dimMin1 = dim - 1;
int k, c, kdim;
int ii, jj, z, k1Neg, k1Plus, cPlus1, cMinus1;
pixel_sum sum;
pixel current_pixel;
      ...
      for (j = 0; j < dim; j += 8){
            for(k = i; k < i + 8; k++){
                  ...
                  for(c = j; c < j + 8; c++){
                        ...
                        cPlus1 = c+1;
                        initialize_pixel_sum(&sum);
                        for(ii = maximum(k1Neg, 0); ii <= minimum(k1Plus, dimMin1); ii++){
                              z = ii * dim;
                              for(jj = maximum(cMinus1, 0); jj <= minimum(cPlus1, dimMin1); jj++){
                                    accumulate_sum(&sum, src[z + jj]);
                              }
                        }
                        assign_sum_to_pixel(&current_pixel, sum);
                        dst[kdim + c] = current_pixel;
                        //assign_sum_to_pixel(dst + (d-c*dim), sum)
                  }
            }
      }
}
